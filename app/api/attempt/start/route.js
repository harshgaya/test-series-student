...
export async function POST(request) {
  try {
    ...
    if (!student) return errorResponse('Please login to attempt this test', 401)

    const { testId } = await request.json()
    if (!testId) return errorResponse('testId required')

    const testIdInt = parseInt(testId)
    if (isNaN(testIdInt)) return errorResponse('Invalid test ID')
      ...
      include: {
        testQuestions: {
          ...
          include: {
            question: {
              include: {
                options:  { orderBy: { orderIndex: 'asc' }, select: { id:true, label:true, optionText:true, optionImageUrl:true, isCorrect:true } },
    ...
    if (test.status !== 'PUBLISHED') return errorResponse('This test is not available', 403)
    ...
    if (Number(test.price) > 0 && !student.freeAccess) {
      const purchase = await prisma.testPurchase.findUnique({
        where: { studentId_testId: { studentId: student.id, testId: testIdInt } },
      })
      if (!purchase) return errorResponse('Please purchase this test first', 403)
    }

    // Resume existing IN_PROGRESS attempt
    const existing = await prisma.testAttempt.findFirst({
      where: { studentId: student.id, testId: testIdInt, status: 'IN_PROGRESS' },
